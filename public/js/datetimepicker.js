var monthFormatter = new Intl.DateTimeFormat("vi-VI", { month: "long" });
var weekdayFormatter = new Intl.DateTimeFormat("vi-VI", { weekday: "long" });
...
// console.log(dates);
  $(displayBoxes[0]).text(dates[0].getDate() + " " + monthFormatter.format(dates[0]));
  $(startDate).val(getDateString(dates[0]));
  $(displayBoxes[1]).text(dates[1].getDate() + " " + monthFormatter.format(dates[1]));
  $(endDate).val(getDateString(dates[1]));
});
...
function updateDateShown() {
  ...
  var dayAndMonth = dates[currentDate].getDate() + " " + monthFormatter.format(dates[currentDate]);
